...
public class BlockService {
	...
	public BlockService() {
		// 新建创始区块
		Block genesisBlock = createNewBlock(1, "1", "1",
	...
	public Block getLatestBlock() {
		return blockchain.size() > 0 ? blockchain.get(blockchain.size() - 1)
				: null;
	}
	...
	public boolean isValidNewBlock(Block newBlock, Block previousBlock) {
		if (previousBlock.getIndex() + 1 != newBlock.getIndex()) {
			...
			// 验证新区块hash值的正确性
			String hash = calculateHash(newBlock.getPreviousHash(),
	...
	private String calculateHash(String previousHash,
			List<Transaction> currentTransactions, int nonce) {
		return CryptoUtil.SHA256(previousHash
				+ JSON.toJSONString(currentTransactions) + nonce);
	}
	...
	public Block mine(String toAddress) {
		...
		// 去除已打包进区块的交易
		List<Transaction> blockTxs = new ArrayList<Transaction>(
				currentTransactions);
		...
		// 创建新的区块
		Block block = createNewBlock(nonce, getLatestBlock().getHash(), hash,
				blockTxs);
		return block;
	}
	...
	public Transaction newCoinbaseTx(String toAddress, String data) {
		...
		TransactionOutput txOut = new TransactionOutput(10,
				wallet.getHashPubKey());
	...
	public Transaction createTransaction(Wallet senderWallet,
			Wallet recipientWallet, int amount) {

		List<Transaction> unspentTxs = findUnspentTransactions(senderWallet
				.getAddress());
		...
		TransactionInput txIn = new TransactionInput(prevTx.getId(), amount,
				null, senderWallet.getPublicKey());
		TransactionOutput txOut = new TransactionOutput(amount,
				recipientWallet.getHashPubKey());
		Transaction transaction = new Transaction(CryptoUtil.UUID(), txIn,
				txOut);
	...
	private List<Transaction> findUnspentTransactions(String address) {
		...
		for (Block block : blockchain) {
			...
			for (Transaction tx : transactions) {
				if (address.equals(CryptoUtil.MD5(tx.getTxOut()
						.getPublicKeyHash()))) {
